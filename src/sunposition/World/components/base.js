...
import { BufferGeometry, Float32BufferAttribute, CylinderGeometry, Mesh, MeshStandardMaterial, MeshBasicMaterial } from 'three'
...
function createBase(params) {
  ...
  function createDirections() {
    fontLoader.load('../fonts/droid_sans_bold.typeface.json', function (font) {
      ...
      text = new Mesh(textGeometry, fontMaterial)
      text.position.z = -params.radius - 4
      text.rotation.x = -Math.PI / 2
      textGeometry.computeBoundingBox()
      text.position.x = -(textGeometry.boundingBox.max.y - textGeometry.boundingBox.min.y) / 2
      // text.rotation.z = - Math.PI/2
      base.add(text)
      textGeometry = new TextGeometry('W', { font, size: 3, height: 0.3 })
      text = new Mesh(textGeometry, fontMaterial)
      text.position.z = params.radius + 4
      text.rotation.x = -Math.PI / 2
    ...
    // Directions arrows
    let arrowVertices = [-5, 0, 0, 0, 0, 10, 0, 0, -10]
  ...
  base.update = () => {
    ...
    base.geometry = new CylinderGeometry(params.radius - 1, params.radius - 1, 2, 180)
